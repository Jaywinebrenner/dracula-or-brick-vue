...
if(!is_sprite){get(_responsive_)&&gauge();get(_preloaded_)&&t.attr({src:reen(reel.substitute(path+images[frame-1],get))});if(!$('#rg-gallery').find('.rg-slider').slider("option","disabled")){$('#rg-gallery').find('.rg-slider').slider('value',frame)}}else{var
...
t.css({height:is_sprite?px(height):null,backgroundSize:size||null});force||t.trigger('imagesChange')}else{t.trigger('stop');$(".rg-slider-btn i").removeClass("fa-pause").addClass("fa-play")}},'setup.fu':function(e){var
...
var totalWidth=$("#gprogress_bar_container").width();$("#gprogress_bar").width(current/width*totalWidth);if(get(_preloaded_)===images&&current>width-1){loaded=!1;$('#rg-gallery').find('.rg-slider').slider('enable');$('#rg-gallery').find('.rg-slider-wrap').css('background','#000000');$('#gprogress_bar_container').hide();$('#rg-gallery').find('.rg-slider-btn i').removeClass('fa-play').addClass('fa-pause');preloader.$.fadeOut(300,function(){preloader.$.css({opacity:1,width:0})})}},'tick.reel':function(e){if(get(_shy_))return;var velocity=get(_velocity_),leader_tempo=leader(_tempo_),monitor=opt.monitor
